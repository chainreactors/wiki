...
> 完整技术变更日志详见 [IoM 更新日志](../../IoM/index.md#更新日志)
...
- [fix] 修复 artifact 构建、上传、大小校验和重复命名相关问题
...
- [fix] 修复 HTTP keepalive polling 导致的信道不稳定与会话假死问题
- [fix] 修复模块执行、第三方模块加载和结果回传中的并发竞争，降低 panic 风险
- [fix] 修复系统信息采集、retry、多目标重连等稳定性问题
- [fix] 修复 `malefic-mutant` HTTP profile 生成后无法编译的问题
