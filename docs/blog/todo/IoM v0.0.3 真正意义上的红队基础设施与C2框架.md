...
- **malefic-srdi (新增)**, 最先进的srdi技术, 最大程度减少PE特征
**基础库:**
- malefic-modules, 各种模块的具体实现, v0.0.3添加了仅30个原生模块, 覆盖service, registry, taskscheduer, token, wmi等常用功能
- malefic-core (新增), 核心库, 实现beacon/bind与modules的交互, 可以通过core快速实现各种不同模板不同需求的implant.
- malefic-proto (新增), 加密与协议库, 定义了implant与server数据交互的协议与加密方式等
...
这个工程还在继续. 在下个版本, 我们将进一步所有对系统api调用不可控的库, 例如wmi, netstat2, sysinfo等库, 所有的关于系统调用的功能, 都将通过win-kit封装的实现了 RashoGate的syscall实现. 
...
我们提供了基于yaml的自动化代码生成器, 能通过yaml快速生成不同场景下的prelude的二进制文件.
...
我们通过rust 内联汇编+rust的features控制, 实现的是shellcode generator的generator , 可以实现高度定制化的shellcode generator
#### loader 
Cobaltstrike提供了一些kit, 其中就有artifact kit, 能控制stager的生成, 并在其中进行一定程度的EDR对抗. 
IoM将提供一套类似的工具, 能调用我们封装过的syscall, inject, cryptor等流程, 快速实现一个shellcode 加载器. 

当然这个加载器不会是唯一选择, 开源的工具不可能在对抗上有优势. 
#### malefic-mutant
刚才提到了implant中多个新的组件, 为了管理这些组件, 以及实现自动化/半自动化编译, 我们将原先的malefic-config升级到malefic-mutant. 
...
- srdi, 通过srdi可以将prelude/beacon 转为shellcode, 变成多阶段加载器的一部分. 
...
另一个弱点则是暂时还没有GUI, 所以下一阶段的工作重点将是GUI. 
...
我们计划给有红队蓝军建设计划的企业提供系统化的基础设施与技术支撑。 