...
ping
```
对 bind 会话执行一次性连通性探测。该命令只发送一次 ping，不会启动后台轮询；如果需要查看普通任务结果，请使用 `wait <task_id>` 或 `fetch_task <task_id>`。
...
polling status
```
为 bind 会话启动或停止后台探测循环。直接执行 `polling` 等价于 `polling start`，同一会话只允许存在一个后台轮询，重复启动会被拒绝。
...
<command> --wait
```
bind 会话执行普通命令时会先主动投递一次任务。等待任务结果时，如果后台 `polling` 已启动，结果会由 polling 持续推进；如果 polling 未启动，`wait` 与 `--wait` 会在等待期间自动发送轻量 ping 来触发结果回收。beacon 会话保持原有等待行为，不会触发 bind ping。
