...
- `load_module` 命令支持多种方式动态加载模块：
    - `load_module --path module.dll` 从本地路径加载模块
    - `load_module --artifact <artifact_name>` 使用服务端已编译好的 artifact 加载
    - `load_module --modules basic,extend` 指定模块列表，由服务端即时编译
    - `load_module --3rd rem` 加载第三方模块（目前支持 rem 和 curl）
    - `load_module --bundle <bundle_name>` 加载指定名称的模块捆绑包

    请注意，如本体已经含有的模块（生成时组装的模块），再次加载将会覆盖该模块的功能，`load_module` 允许覆盖本体功能
- `refresh_module`  命令将会卸载所有动态加载的模块， 包括覆盖掉的本体模块， 一切模块将恢复成编译时的初始状态

> client的使用请见[使文档module部分](/IoM/manual/manual/implant/#list_module)
### addon 管理
...
> client的使用请见[使文档addon部分](/IoM/manual/manual/implant/#list_addon)
...
> client的使用请见[使文档task部分](/IoM/manual/manual/implant/#tasks)
## Module
