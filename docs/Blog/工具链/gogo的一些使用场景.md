...
前几天公开了gogo, 但如果没有结合场景进行实践, 可能很难感受到gogo的强大之处. 依旧将gogo当作github上同类工具的代替, 或者因为gogo没有集成服务的弱口令爆破认为gogo不够方便. 

所以后续还有一系列关于gogo的各种场景下的使用, 以及二次开发的文章.
...
gogo通常作为唯二(另一个是shellcode loader)需要在内网落地的工具, 在之后我们也会提供更优的解决方案实现gogo不落地情况下的高并发扫描. 如果你的C2实现的反射加载PE, gogo也可以不落地, 节省了免杀的成本.
