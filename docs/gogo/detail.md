...
??? important "通过finger配置的漏洞"
    | 通过finger配置的vuln类型漏洞 | 通过finger配置的信息泄露漏洞 |
    | ---------------------------- | ---------------------------- |
    | k8s_api_unauth               | 普元EOS_console_leak         |
    | elasticsearch_unauth         | weblogic_console_leak        |
    | hadoop_unauth                | sourcemap-leak               |
    | docker_unauth                | swagger leak                 |
    | etcd_unauth                  | druid leak                   |
    | flink_unauth                 | iis Directory traversal      |
