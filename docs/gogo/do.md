...
`gogo --workflow 172b `  发现172.16.0.0/12中存活的b段.
...
`gogo -j .172.16.126.116_12_top1_ss_bcidr.dat1 --workflow s`   
...
而如果当前的网段为192段这种B类地址, 那么`--mod ss`就无法发挥作用.
...
`gogo -j bcidr.dat --mod s --no --af`

如果需要合并2,3两步则更为简单. 

`gogo --workflow 10c` 探测10段内存活的c段

或者类似的`gogo --workflow 172c` or `gogo --workflow 192c`

如果是自定义网段, 可以使用 `gogo --workflow sc -i 111.1.1.1/8`
...
`gogo --workflow 10` 默认开启了--ping, 如果禁止了icmp协议, 需要替换成`gogo --workflow 10noping` .

172与192段同理. 当然自定义网段可以根据实际情况选用, 

`gogo --workflow ss -i 111.1.1.1/8`

or

`gogo --workflow s -i 111.1.1.1/16`

如果网段小于16则建议使用`--workflow ss` ,如果大于等于16则建议使用`--workflow s`.
...
`gogo --workflow 10 -ev` 172, 192以及自定义网段同理. 
...
supersmart下网关常见端口为 `80,23,22` (实际上所有ss相关的workflow在noping模式下会自动将端口探针修改为80, 但为了适配网关的规律, 建议添加上23,22端口):
 ...
 `gogo -i 10.0.0.0/8 --workflow ssnoping --sp 80,23,22`    

smart下默认的探针端口为80,不需要修改.  

`gogo -i 172.16.1.1/16 --workflow snoping` 
...
这种情况如果使用fscan, 将导致直接忽略掉这些ip/网段, 因为fscan默认的扫描策略强依赖于icmp协议, 并没有提供代替方案, 因此对使用者来说, 漏掉的这些资产是无感的. fscan的扫描策略类似`--workflow ss`模式的gogo的第一阶段. 

如果使用`gogo -i 172.16.1.1/16 --workflow snoping` 则可以避免这种情况的同时保证扫描耗时不会有指数级的提升.  依旧能在半个小时左右扫完一个b段. 
