...
如果想深入了解gogo, 需要理解gogo面临的问题, 以及gogo的设计理念与解决方案, 也就是[设计](/wifi/gogo/design)与[细节](/wiki/gogo/detail)两章. 就能理解到为何gogo是为红队而设计的.
...
gogo对内存的占用主要是go的goroutine自身的栈, gogo本身已经经可能的复用goroutine了.  通过几个版本的针对性优化, 基本能在超低性能的机器上正常运行.
...
后续作为一个公开的工具, gogo也不会在免杀上做后续的对抗. 可能需要使用者自行进行免杀.
...
gogo已经尽可能在能做到的情况下做到极限, 但受限于开发者的想法与遇到的场景不同, 还是有些功能或者场景无法覆盖, 欢迎提交issue/pr.
...
漏洞扫描相关功能从nuclei学到了不少东西, dsl语法也直接使用了nuclei, 但为了更加适配内网场景, 做了不少改动, 并不能完全兼容.  考虑到了内网环境, 我删除了一部分不必要的功能(动态dsl, oast等), 以简化二进制文件大小.   也添加了一些功能, 详情请见[拓展nuclei poc](/wiki/gogo/extension/#poc)

绝大多数poc都可以从nuclei中移植poc, 只需要做一些简单修改与测试便可以快速加入到gogo中.  不过为了保证poc质量以及内网使用场景, 加入到gogo中的漏洞需要一定的筛选.

这里的考量是, 红队场景中, 对于各种oa, cms, 不少人都手握0day, 只需要做指纹识别与版本识别, 需要打poc的情况不太多. 

对于一些设备, 开源组件, 真正用到nuclei poc的地方是默认口令爆破, 需要自动化探测默认密码. **与其在文档中收集设备的默认密码, 不如直接转换成poc, 自动化的帮你爆破.** 
...
gogo最核心的几块功能就是刚才提到的, 但实际上还有不少细节我没有一一介绍, 使用者可以在实战中慢慢体会, 也欢迎提出新的需求以及pr.
