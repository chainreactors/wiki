...
比起一些项目直接调用nuclei包, gogo的这种方式去掉了**所有的依赖**, 并最大程度的实现了nuclei的引擎, 缺点是, dsl引擎的版本将会略微落后于nuclei本体. 考虑到gogo的使用场景, 这种落后并不影响功能, 因此是可以接受的代价. 
...
**gogo不是缝合怪**, 每个细节都经过漫长的反复的改动, 最终呈现出来现在的样子.  或许在很多地方还存在缺陷, 但在功能上已经达到了此类工具的极限. 
