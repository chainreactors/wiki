...
# 或编译为独立模块 DLL
malefic-mutant generate modules "your_module"
...
```bash
# 生成配置（指定模块列表）
malefic-mutant generate modules "execute_powershell execute_assembly execute_bof"

# 编译
malefic-mutant build modules --target x86_64-pc-windows-gnu
```

编译产物：`target/<target_triple>/release/modules.dll`
...
| `full` | fs_full + execute_full + net_full + sys_full | 全部功能（默认） |
