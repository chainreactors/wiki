...
进入 release 离线包的 `source_code/` 后，使用包内配套的 `malefic-mutant` 生成 pulse 配置：

```bash
./bin/malefic-mutant generate pulse
```
...
```bash
# Windows x64
./bin/malefic-mutant build pulse -c implant.yaml -t x86_64-pc-windows-gnu

# Windows x86
./bin/malefic-mutant build pulse -c implant.yaml -t i686-pc-windows-gnu
```
...
```bash
./bin/malefic-mutant build pulse --shellcode -c implant.yaml -t x86_64-pc-windows-gnu
```
...
推荐使用 Docker 进行交叉编译。release 离线包已经包含 vendor 依赖，不需要额外挂载 Cargo cache：

```bash
docker run -v "$(pwd):/root/src" --rm -it ghcr.io/chainreactors/malefic-builder:latest \
  sh -lc "cd /root/src && chmod +x ./bin/malefic-mutant && ./bin/malefic-mutant generate pulse && ./bin/malefic-mutant build pulse -c implant.yaml -t x86_64-pc-windows-gnu"
```
...
# 2. 使用 SRDI 转换
./bin/malefic-mutant tool srdi -i pulse.exe -o pulse.bin
