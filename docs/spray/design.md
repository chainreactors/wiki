...
!!! info "背景"
	绝大多数目录爆破工具, 它们过度简化了现实世界中可能遇到的场景, 导致只能在理想状况下实现一定程度的过滤无效页面, 面对复杂的真实场景, 他们的这种方式既会导致误报, 也会导致漏报. 而spray要做的是尽可能减少误报与漏报. 
...
## 字典构造
!!! info "背景"
	绝大部分工具将字典生成与爆破的行为分开了, 造成了一定程度的割裂. 很多人会觉得有工具没合适的字典, 或者想拿一个字典批量爆破一批目标却找不到合适的工具. 我也正是因为遇到了这样的困境才选择去实现一个新的工具. 因此spray从设计上就考虑到了这种割裂, 并尝试解决这种割裂. 
