...
!!! note "fasthttp坑点"
	它复用了几乎所有的bytes buf, 因此releaseResponse之后, 可能buf会被复用, 导致buf的数据被覆盖. 这个bug我花费了好几天时间才定位到. 

### http

在大多数场景下, http协议对性能的影响最关键的就是`keep-alive`, 这个功能需要http1.1之后才支持. 好在绝大多数服务现在都已经支持了. 这也是spray性能提升最大因素. 

在python中, 需要手动控制keep-alive的并发池去复用. 

但好在go中不论时`fasthttp`还是`net/http`都内置自带了链接的复用. 
